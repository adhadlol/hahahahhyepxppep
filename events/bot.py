...
# Removed: from patches import functions
import hashlib

# Dummy functions replacement for missing patches module
class functions:
    @staticmethod
    async def image_hash(image: bytes) -> str:
...
class Bot(commands.Bot):
    async def setup_hook(self):
        self.db = await aiosqlite.connect("database.db")  # Use your actual DB file
        await self.db.execute("PRAGMA foreign_keys = ON")  # Enable foreign keys
...
async def servers_check(bot: commands.Bot):
    async with bot.db.execute("SELECT guild_id FROM authorize") as cursor:
        authorized_guilds = {row[0] for row in await cursor.fetchall()}
    return [await guild.leave() for guild in bot.guilds if guild.id not in authorized_guilds and guild.id != 1098449519101870182]
...
class BotCog(commands.Cog): 
    ...
    async def join_message(self, guild: discord.Guild):
        async with self.bot.db.execute("SELECT * FROM gblacklist WHERE guild_id = ?", (guild.id,)) as cursor:
            check = await cursor.fetchone()
        async with self.bot.db.execute("SELECT * FROM authorize WHERE guild_id = ?", (guild.id,)) as cursor:
        ...
        if channel := discord.utils.find(lambda c: c.permissions_for(guild.me).embed_links, guild.text_channels):
            embed = Embed(
    ...
    async def avatar_update(self, before: discord.User, after: discord.User):
        ...
        with contextlib.suppress(discord.HTTPException):
            ...
            await self.bot.db.execute(
                "INSERT OR IGNORE INTO avatars (user_id, avatar, hash, timestamp) VALUES (?, ?, ?, ?)",
                (before.id, message.attachments[0].url, image_hash, int(discord.utils.utcnow().timestamp())),
            )
            await self.bot.db.commit()  # Needed in SQLite to save changes
