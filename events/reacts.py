        ...
        try:
            ...
            if check:
                ...
                chec = await self.bot.db.fetchrow(
                    "SELECT * FROM starboardmes WHERE guild_id = ? AND channel_message_id = ? AND message_id = ?",
                ...
                if chec is not None:
                    ...
                    try:
                        total = sum(r.count for r in message.reactions if str(r.emoji) == str(payload.emoji))
                        return await messa.edit(content=f"{payload.emoji} **#{total}** <#{payload.channel_id}>")
                    except Exception:
                        pass
                elif chec is None:
                    channel = payload.member.guild.get_channel(check['channel_id'])
                    if not message.embeds:
                        embed = discord.Embed(color=self.bot.color, description=message.content, timestamp=message.created_at.now())
                    else:
                        embed = discord.Embed(
                    ...
                    async def idk():
                        # Note: In this closure, 'reaction' is not defined. 
                        # It may be intended to capture a reaction from the for-loop below.
                        ...
                        await self.bot.db.execute(
                            "INSERT INTO starboardmes VALUES (?, ?, ?, ?, ?)",
                            (payload.guild_id, channel.id, payload.channel_id, mes.id, payload.message_id)
                        )
