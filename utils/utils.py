...
class InvokeClass:
    ...
    async def invoke_send(ctx: commands.Context, member: Union[discord.User, discord.Member], reason: str):
        res = await ctx.bot.db.fetchrow(
            "SELECT embed FROM invoke WHERE guild_id = ? AND command = ?",
            ...
            try:
                x = await EmbedBuilder.to_object(
                    EmbedBuilder.embed_replacement(
                        member,
                        InvokeClass.invoke_replacement(member, code.replace("{reason}", reason))
                    )
                )
                await ctx.reply(content=x[0], embed=x[1], view=x[2])
            except Exception:
                await ctx.reply(EmbedBuilder.embed_replacement(member, InvokeClass.invoke_replacement(member, code.replace("{reason}", reason))))
            return True
        return False
        ...
        if res:
            ...
            if embed == "none":
                await ctx.bot.db.execute(
                    "DELETE FROM invoke WHERE guild_id = ? AND command = ?",
                    (ctx.guild.id, ctx.command.name)
                )
