...
class Reposter:
    ...
    async def repost_twitter(self, bot: commands.Bot, message: discord.Message, url: str):
        ...
        if len(tweet.media.videos) >= 1: file = discord.File(fp=await self.read_file_from_url(tweet.media.videos[0].variants[1].url), filename='yep-twitter.mp4')
    ...
    async def repost_youtube(self, bot: commands.Bot, message: discord.Message, url: str):
        ...
        file = discord.File(fp=await self.read_file_from_url(formats[0]['url']), filename='yep-youtube.mp4')
        ...
        async with locks[message.guild.id]:  
         ...
         async with message.channel.typing():       
          ...
          if x['data'].get("images"):
            ...
            video = x["data"]["play"]
            file = File(fp=await session.getbyte(video), filename="yeptiktok.mp4")
            embed = Embed(
              color=bot.color,
