...
from bot.helpers import YepContext
        ...
        async def on_submit(self, interaction: discord.Interaction):
            try: 
                ...
                if emoji.is_emoji(self.name.value):
                    ...
                    emoj = emojis[0]
                    fmt = ".gif" if emoj[0] == "a" else ".png"
                    url = "https://cdn.discordapp.com/emojis/{}{}".format(emoj[2], fmt)
                ...
                return await interaction.client.ext.error(interaction, "Unable to change the role icon", ephemeral=True)
    ...
    class Color(discord.ui.Modal, title="change the role colors"):
        ...
        async def on_submit(self, interaction: discord.Interaction):
            try: 
                ...
                return await interaction.client.ext.error(interaction, "Unable to change the role color", ephemeral=True)

class OwnerConfig:
    async def send_dm(ctx: commands.Context, member: discord.Member, action: str, reason: str): 
        embed = discord.Embed(
            color=ctx.bot.color,
            description=f"You have been **{action}** in every server yep is in.\n{f'**Reason:** {reason}' if reason != 'No reason provided' else ''}"
        )
...
class ValidWebhookCode(commands.Converter):
    async def convert(self, ctx: YepContext, argument: str):
        check = await ctx.bot.db.fetchrow(
            "SELECT * FROM webhook WHERE guild_id = ? AND code = ?",
            (ctx.guild.id, argument)
        )
        if not check:
...
class Mod: 
    def is_mod_configured():
        async def predicate(ctx: commands.Context):
            check = await ctx.bot.db.fetchrow(
                "SELECT * FROM mod WHERE guild_id = ?",
                (ctx.guild.id,)
            )
            if not check:
...
class Timezone(object):
    ...
    async def get_user_lat_long(self, member: discord.Member):
        check = await self.bot.db.fetchrow(
            "SELECT * FROM timezone WHERE user_id = ?",
            (member.id,)
        )
        if check is None:
            return None
    ...
    async def tz_set_cmd(self, ctx: Context, location: str):
        await ctx.typing()
        geolocator = Nominatim(user_agent="Mozilla/5.0 (Linux; Android 8.0.0; SM-G955U Build/R16NW) "
                                         "AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.141 Mobile Safari/537.36")
        lad = location
        location_obj = geolocator.geocode(lad)
        ...
        obj = TimezoneFinder()
        result = obj.timezone_at(lng=location_obj.longitude, lat=location_obj.latitude)
        if not check:
            await self.bot.db.execute(
                "INSERT INTO timezone VALUES (?, ?)",
                (ctx.author.id, result)
            )
        else:
            await self.bot.db.execute(
                "DELETE FROM timezone WHERE user_id = ?",
                (ctx.author.id,)
            )
            await self.bot.db.execute(
                "INSERT INTO timezone VALUES (?, ?)",
    ...
    async def get_user_timezone(self, ctx: Context, member: discord.Member):
        if await self.timezone_request(member) is None:
            if member.id == ctx.author.id:
                return await ctx.warning(f"You don't have a **timezone** set. Use `{ctx.clean_prefix}tz set` command instead")
            else:
                return await ctx.warning(f"**{member.name}** doesn't have their **timezone** set")
        if member.id == ctx.author.id:
            return await self.timezone_send(ctx, f"Your current time: **{await self.timezone_request(member)}**")
        else:
            return await self.timezone_send(ctx, f"**{member.name}'s** current time: **{await self.timezone_request(member)}**") 

class Player(pomice.Player):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.ctx: YepContext = None
        self.queue = pomice.Queue()
        self.loop: bool = False
        self.current_track: pomice.Track = None
        self.awaiting = False

    def set_context(self, ctx: YepContext):
        self.context = ctx
