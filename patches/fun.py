...
class RockPaperScissors(discord.ui.View): 
    ...
    async def action(self, interaction: discord.Interaction, selection: str): 
        if interaction.user.id != self.ctx.author.id:
            return await interaction.client.ext.warning(interaction, "This is not your game", ephemeral=True)
        # Note: The bot's selection string here is ambiguous.
...
class MarryView(discord.ui.View): 
    ...
    async def yes(self, interaction: discord.Interaction, button: discord.ui.Button): 
        if interaction.user == self.ctx.author:
            return await interaction.client.ext.warning(interaction, "You can't accept your own marriage".capitalize(), ephemeral=True)
        elif interaction.user != self.member:
            return await self.ctx.bot.ext.warning(interaction, "You are not the author of this embed".capitalize(), ephemeral=True)
        else:   
            await interaction.client.db.execute(
                "INSERT INTO marry VALUES (?, ?, ?)",
                (self.ctx.author.id, self.member.id, datetime.datetime.now().timestamp())
            )
            embe = discord.Embed(
                color=interaction.client.color, 
                description=f"<a:milk_love:1208459088069922937> **{self.ctx.author}** successfully married with **{self.member}**"
            )
    ...
    async def no(self, interaction: discord.Interaction, button: discord.ui.Button): 
        if interaction.user == self.ctx.author:
            return await self.ctx.bot.ext.warning(interaction, "You can't reject your own marriage".capitalize(), ephemeral=True)
        elif interaction.user != self.member:
            return await self.ctx.bot.ext.warning(interaction, "You are not the author of this embed".capitalize(), ephemeral=True)
        else:                         
            embe = discord.Embed(
...
class DiaryModal(discord.ui.Modal, title="Create a diary page"): 
    ...
    async def on_submit(self, interaction: discord.Interaction): 
        ...
        await interaction.client.db.execute(
            ...
            (interaction.user.id, self.text.value, self.titl.value, date)
        )
        embed = discord.Embed(
            color=interaction.client.color, 
    ...
    def check_joint():
        async def predicate(ctx: commands.Context): 
            check = await ctx.bot.db.fetchrow(
                "SELECT * FROM joint WHERE guild_id = ?",
                (ctx.guild.id,)
            )
            if not check:
                await ctx.bot.ext.error(ctx, f"This server **doesn't** have a **joint**. Use `{ctx.clean_prefix}joint toggle` to get one")    
    ...
    def joint_owner(): 
        async def predicate(ctx: commands.Context): 
            ...
            if check["holder"] != ctx.author.id:
                await ctx.warning(f"You don't have the **joint**. Steal it from <@{check['holder']}>")
